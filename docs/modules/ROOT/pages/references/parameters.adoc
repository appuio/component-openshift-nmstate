...
NNCPs support defining tehe maximum number of nodes where configurations are applied simultaneously via `spec.maxUnavailable`.
...
Each key value pair in the parameter is translated into a NNCP which configures one or more interfaes which are assigned the desired IP(s).
The component uses the keys as `metadata.name` for the resulting NNCPs.
Keys with value `null` are skipped.
